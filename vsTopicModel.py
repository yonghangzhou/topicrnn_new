...
class vsTopic(object):
  ...
  def forward(self, inputs,params, mode="Train"):
    ...
    with tf.name_scope("RNN_CELL"):
      emb = tf.nn.embedding_lookup(self.embedding, inputs["tokens"])
      def make_cell():
        cell = LSTMCell(self.num_hidden,dropout=params["dropout"])
        # cell = Dropout(cell,rate=1-self.keep_prob)
        return cell      
        cells =  StackedRNNCells([make_cell() for _ in range(self.num_layers)])        
      rnn_outputs, final_output = tf.nn.dynamic_rnn(cell, inputs=emb, sequence_length=inputs["length"], dtype=tf.float32)
    ...
    with tf.name_scope("token_loss"):     
      ...
      if params["beta_batch"]==1:
        token_logits = tf.expand_dims(tf.layers.dense(rnn_outputs, units=self.vocab_size, use_bias=False),2) + tf.expand_dims(1-stop_indicator,-1)*tf.contrib.layers.batch_norm(tf.expand_dims(self.beta,0))        
      elif params["beta_batch"]==0:
        token_logits = tf.expand_dims(tf.layers.dense(rnn_outputs, units=self.vocab_size, use_bias=False),2) + tf.expand_dims(1-stop_indicator,-1)*tf.expand_dims(self.beta,0)
    ...
    tf.summary.scalar(tensor=phi_theta_kl_loss, name="phi_theta_kl_loss")    
    tf.summary.scalar(tensor=indicator_loss, name="indicator_loss")
    tf.summary.scalar(tensor=theta_kl_loss, name="theta_kl_loss")
    tf.summary.scalar(tensor=total_loss, name="total_loss")
    tf.summary.scalar(tensor=token_ppl, name="token_ppl")
